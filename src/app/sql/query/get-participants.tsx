...
export async function getParticipants(eventCategoryId: number, teamId: number|null = null) : Promise<Participant[]>{
    try {
        unstable_noStore();
        var event = teamId === null 
            ? await sql<Participant>`SELECT * FROM Participants WHERE EventCategoryId = ${eventCategoryId}`
            : await sql<Participant>`SELECT * FROM Participants WHERE EventCategoryId = ${eventCategoryId} AND TeamId = ${teamId}`
